...
void Cinta::EliminarvalSimbolos(TStringGrid *MatrizDeEstados,char valores){
	if ((!Vacia())&&(Pertenece(valores))) {
		...
		for (int j = i; j < cabezal; j++) {
			...
			MatrizDeEstados->Cells[cabezal+1][0]=VSimbolo[j];
		}
		cabezal--;
...
void Cinta::LlenarUnCombobox(TComboBox *CoBox){
	for (int j = 0; j <=cabezal; j++) {
		char Simbolo=VSimbolo[j];
		String s(&Simbolo, 1);
		CoBox->Items->Add(s);
	}
}
